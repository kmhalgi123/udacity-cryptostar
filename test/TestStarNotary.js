...
it('can add the star name and star symbol properly', async() => {
    ...
    let instance = await StarNotary.deployed();
    await instance.createStar('Awesome Star!', tokenId, {from: accounts[0]});
    assert.equal('StarToken', await instance.name.call());
    assert.equal('TKN', await instance.symbol.call());
});
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('Equally Awesome Star!', tokenId2, {from: accounts[1]});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let instance = await StarNotary.deployed();
    await instance.createStar('Awesome Star!', tokenId, {from: accounts[0]});
    let val = await instance.lookUptokenIdToStarInfo(tokenId);
    assert.equal('Awesome Star!', val)
});